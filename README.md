...
Before running `setup.sh`, create `/home/pitv/waiting-room/config.env` with:
...
Leave `APP_SECRET` blank — setup.sh will generate a random value automatically. Only change `ADMIN_PASS` to something secure. Do not change `APP_SECRET` after the first run as it signs session cookies and changing it will log everyone out.
